...
export default function Footer({ showCta = true }: FooterProps) {
  ...
  return (
    <footer className="vizit-footer relative overflow-hidden border-t border-[#d39a43]/20 bg-[#fbf5ed] transition-colors dark:border-[#edc982]/15 dark:bg-[#120b14]">
      ...
      <div className="relative mx-auto max-w-[1320px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        {showCta ? (
          <motion.div
          ...
          >
            ...
            <div className="relative grid gap-7 px-6 py-7 sm:px-8 sm:py-8 lg:px-10 xl:grid-cols-[1fr_auto] xl:items-center">
              <div>
                ...
                <h2 className="vizit-display mt-4 max-w-2xl text-2xl font-bold tracking-[-0.03em] text-white sm:text-3xl lg:text-[34px]">
                  {t("footer.title")}
                </h2>
        ...
        <div className={`${showCta ? "mt-10" : "mt-0"} grid gap-7 md:grid-cols-2 xl:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr]`}>
          <div>
            <Link to="/" className="vizit-footer-brand flex items-center gap-3">
              <VizitLogo />
              <div>
                <div className="text-xs text-[#665568] dark:text-white/70">{t("footer.platformTagline")}</div>
              </div>
            </Link>

            <p className="mt-4 max-w-md text-sm leading-7 text-[#5f5062] dark:text-white/75">
