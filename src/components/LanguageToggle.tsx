    ...
    <div ref={rootRef} className={cn("vizit-language-root relative isolate", compact ? "inline-flex" : "w-full", className)}>
      {compact ? (
        <>
          ...
          {open ? (
            <div role="menu" aria-label={{ hy: "Ընտրել լեզուն", ru: "Выбрать язык", en: "Choose language" }[locale]} className="vizit-language-menu absolute right-0 top-[calc(100%+8px)] z-[120] grid min-w-[172px] gap-1 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl dark:border-white/10 dark:bg-slate-950">
              {options.map((option) => (
                <button key={option.value} role="menuitemradio" aria-checked={option.value === locale} type="button" onClick={() => { setLocale(option.value); setOpen(false); }} className={cn("vizit-language-option flex items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-bold text-slate-700 dark:text-white", option.value === locale ? "is-active bg-[var(--vz-accent-soft)] text-[var(--vz-brand-text)] dark:bg-[var(--vz-accent-soft)] dark:text-[var(--vz-brand-text)]" : "hover:bg-slate-100 dark:hover:bg-white/10")}>
                  <FlagIcon locale={option.value} />
                  <span className="min-w-7 text-xs font-extrabold tracking-[0.08em] text-[var(--vz-brand-text)] dark:text-[var(--vz-brand-text)]" aria-hidden="true">{option.short}</span>
                  {option.label}
                </button>
              ))}
            </div>
        ...
        <div className="grid w-full grid-cols-3 gap-2 rounded-2xl border border-slate-200 bg-white/70 p-2 dark:border-white/10 dark:bg-white/[0.04]">
          {options.map((option) => (
            <button key={option.value} type="button" onClick={() => setLocale(option.value)} aria-label={option.label} className={cn("flex h-14 min-w-0 items-center justify-center gap-2 rounded-xl border leading-none transition", option.value === locale ? "border-[var(--vz-line-strong)] bg-[var(--vz-accent-soft)] text-[var(--vz-brand-text)] shadow-sm dark:border-[#245171] dark:bg-[var(--vz-accent-soft)] dark:text-[var(--vz-brand-text)]" : "border-transparent hover:bg-slate-100 dark:hover:bg-white/10")}>
              <FlagIcon locale={option.value} />
