...
function extractBusinessList(payload: unknown): PublicDirectoryBusiness[] {
    ...
    return list.map((item: PublicDirectoryBusiness) => ({
...
function extractMapPins(payload: unknown): PublicMapPin[] {
    const data = payload as any;
    const list = Array.isArray(data?.data) ? data.data : Array.isArray(data?.pins) ? data.pins : Array.isArray(data) ? data : [];

    return list
        .map((pin: any) => ({
            business_id: Number(pin.business_id ?? pin.id),
            name: String(pin.name ?? ""),
            slug: String(pin.slug ?? ""),
            vertical: pin.vertical ?? null,
            category_slug: pin.category_slug ?? pin.category?.slug ?? null,
            category_name: pin.category_name ?? pin.category?.name ?? pin.category?.name_hy ?? null,
            icon: pin.icon ?? null,
            location_id: Number(pin.location_id ?? pin.location?.id ?? pin.id),
            location_name: pin.location_name ?? pin.location?.name ?? null,
            address: pin.address ?? pin.location?.address ?? null,
            ...
            booking_url: pin.booking_url ?? (pin.slug ? `/book/${pin.slug}` : null),
        }))
