        ...
        <section id="vizit-business-hero" className="vizit-business-hero relative overflow-hidden px-4 pb-20 pt-32 sm:px-6 sm:pt-40 lg:px-8 lg:pb-28">
          ...
          <motion.div variants={staggerContainer(0.08, 0.04)} initial="hidden" animate="show" className="relative mx-auto grid max-w-[1240px] gap-12 lg:grid-cols-[1.02fr_0.98fr] lg:items-center">
            ...
            <motion.div variants={fadeUp} className="relative mx-auto w-full max-w-[620px]">
              ...
              <div className="vizit-business-preview relative overflow-hidden rounded-[34px] border p-3 backdrop-blur-xl sm:p-5">
                <div className="flex items-center justify-between px-2 pb-4 pt-1 sm:px-3">
                  <div><div className="vizit-business-preview-kicker text-xs font-bold uppercase tracking-[0.16em]">{text.preview}</div><div className="vizit-business-preview-title mt-1 text-lg font-bold">Vizit workspace</div></div>
                  <div className="vizit-business-preview-icon grid h-11 w-11 place-items-center rounded-2xl"><LayoutDashboard className="h-5 w-5" /></div>
                </div>
                <div className="grid gap-3 sm:grid-cols-[150px_1fr]">
                  ...
                  <div className="vizit-business-preview-panel rounded-[24px] border p-4 sm:p-5">
                    <div className="flex items-center justify-between"><div><div className="vizit-business-preview-label text-xs font-semibold">{text.today}</div><div className="vizit-business-preview-count mt-1 text-2xl font-bold">3</div></div><div className="vizit-business-preview-calendar rounded-2xl p-3"><CalendarDays className="h-5 w-5" /></div></div>
                    <div className="mt-5 space-y-2.5">
                      {text.schedule.map(([time, service, staff]) => <div key={`${time}-${service}`} className="vizit-business-preview-event grid grid-cols-[52px_1fr] gap-3 rounded-2xl border px-3 py-3"><div className="vizit-business-preview-time flex items-center gap-1 text-xs font-bold"><Clock3 className="h-3.5 w-3.5" />{time}</div><div className="min-w-0"><div className="vizit-business-preview-service truncate text-sm font-bold">{service}</div><div className="vizit-business-preview-meta mt-1 flex items-center justify-between gap-2 text-[11px]"><span>{staff}</span><span className="vizit-business-preview-status rounded-full px-2 py-1 font-semibold">{text.confirmed}</span></div></div></div>)}
                    </div>
                  </div>
                </div>
              </div>
