...
function InteractiveBusinessMap({
  ...
  const pointsKey = useMemo(
    () => pins.map((pin) => `${pin.locationId}:${pin.lat.toFixed(5)},${pin.lng.toFixed(5)}`).join("|"),
    [pins],
  );
  ...
  }, [pointsKey, userLocation?.lat, userLocation?.lng]);
...
function MobileStats({ stats }: { stats: { total: number; services: number; staff: number } }) {
  const { t } = useLanguage();
  const items = [
    ...
    <motion.div variants={fadeUp} className="mt-7 grid grid-cols-3 gap-2 xl:hidden">
      {items.map(({ value, label, Icon }) => (
        <div key={label} className="rounded-[18px] border border-white/10 bg-white/[0.08] p-3 text-center backdrop-blur-2xl sm:p-4">
          ...
          <div className="mt-1 text-[10px] font-bold leading-4 text-white/64 sm:text-xs">{label}</div>
        </div>
      ))}
    </motion.div>
  );
...
export default function Index() {
  ...
  const allBusinesses = businessesQ.data ?? [];
      ...
      <main>
        <section className="relative overflow-hidden bg-[#050816] px-5 pb-8 pt-[108px] sm:px-8 sm:pt-[126px] lg:pb-0">
          ...
          <motion.div variants={stagger} initial="hidden" animate="visible" className="relative mx-auto grid max-w-[1320px] gap-8 xl:grid-cols-[1fr_610px] xl:items-end">
            ...
            <HeroVisual businessCount={businessesQ.isLoading ? "..." : stats.total} servicesCount={businessesQ.isLoading ? "..." : stats.services} staffCount={businessesQ.isLoading ? "..." : stats.staff} />
          </motion.div>
        </section>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.16 }} className="mx-auto max-w-[1320px] rounded-[28px] border border-white/10 bg-[#07101f] p-6 shadow-[0_30px_100px_rgba(0,0,0,0.30)] sm:rounded-[34px] sm:p-8 lg:p-10">
            ...
            <motion.div variants={fadeUp} className="mt-6 grid gap-3 rounded-[22px] border border-white/10 bg-white/[0.055] p-4 sm:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                { value: businessesQ.isLoading ? "..." : stats.staff, label: t("stats.staff"), Icon: Users },
                { value: businessesQ.isLoading ? "..." : stats.categories, label: t("stats.categories"), Icon: Star },
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        active ? "border-white bg-white text-slate-950" : "border-white/12 bg-white/[0.06] text-slate-300 hover:bg-white/[0.11] hover:text-white",
                      )}
                    >
                      {label}
                    </button>
