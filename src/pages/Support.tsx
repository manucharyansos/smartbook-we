        ...
        <section className="px-4 pb-16 sm:px-6 lg:px-8">
          <motion.div variants={staggerContainer(0.08, 0.05)} initial="hidden" animate="show" className="mx-auto max-w-7xl">
            <motion.div variants={fadeUp} className="text-center">
              ...
              <h1 className="mt-5 text-3xl font-semibold tracking-tight text-slate-950 sm:mt-6 sm:text-5xl lg:text-6xl">{text.title}</h1>
              <p className="mx-auto mt-4 max-w-2xl text-lg leading-8 text-slate-600">{text.intro}</p>
            </motion.div>
            ...
            <div className="mt-12 grid gap-6 2xl:grid-cols-[0.95fr_1.05fr]">
              <motion.div variants={scaleIn} className="rounded-[32px] border border-white/70 bg-white/88 p-6 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-semibold text-slate-900">{text.team}</h2>
                    <p className="text-sm text-slate-500">{text.teamIntro}</p>
                  </div>
                </div>

                <div className="mt-6 space-y-4">
                  {channels.map((item) => (
                    <motion.a key={item.label} href={item.href} target={item.external ? "_blank" : undefined} rel={item.external ? "noreferrer" : undefined} variants={fadeUp} {...hoverLift} className="block rounded-[24px] border border-slate-200 bg-slate-50/80 p-5 transition hover:border-violet-200 hover:bg-violet-50/60">
